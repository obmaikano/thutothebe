...
import { createCurriculum } from '../curriculumSlice';
...
import { CreateCurriculumRequest } from '../../../api/services/curriculumApi';
import { Save, X } from 'lucide-react';

const CreateCurriculumModal: React.FC = () => {
  ...
  const [formData, setFormData] = useState({
    title: '',
    description: '',
    curriculumType: 'SCHOOL_SPECIFIC' as CreateCurriculumRequest['curriculumType'],
    gradeLevel: 'STANDARD_1' as CreateCurriculumRequest['gradeLevel'],
    status: 'DRAFT' as CreateCurriculumRequest['status'],
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await dispatch(createCurriculum(formData as CreateCurriculumRequest)).unwrap();
      dispatch(closeModal({}));
    } catch (error: any) {
      setError(error.message || 'Failed to create curriculum');
    } finally {
      setIsLoading(false);
    }
  };
