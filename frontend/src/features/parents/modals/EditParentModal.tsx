...
import { updateParentProfile, fetchParents } from '../parentsSlice';
...
const EditParentModal: React.FC<EditParentModalProps> = ({ extraObject }) => {
  ...
  useEffect(() => {
    if (extraObject?.parent) {
      setFormData({
        firstName: extraObject.parent.firstName,
        lastName: extraObject.parent.lastName,
        email: extraObject.parent.email,
        surname: extraObject.parent.surname,
        gender: extraObject.parent.gender,
        nationality: extraObject.parent.nationality,
        dateOfBirth: extraObject.parent.dateOfBirth,
        identityNumber: extraObject.parent.identityNumber,
        birthCertificateNumber: extraObject.parent.birthCertificateNumber,
        qualification: extraObject.parent.qualification,
        active: extraObject.parent.active
      });
    }
  }, [extraObject]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!extraObject?.parent?.id) {
      setErrors({ submit: 'Parent ID is required' });
      return;
    }

    setIsSubmitting(true);
    try {
      await dispatch(updateParentProfile({ id: extraObject.parent.id, parentData: formData })).unwrap();
      setIsSuccess(true);
  ...
  if (!extraObject?.parent) {
    return (
      <div className="text-center py-8">
        ...
        <p className="text-gray-600 mb-4">No parent information was provided for editing.</p>
        <button
          onClick={handleClose}
  ...
  if (isSuccess) {
    return (
      <div className="text-center py-8">
        ...
        <h3 className="text-lg font-semibold text-gray-900 mb-2">Parent Updated Successfully!</h3>
        <p className="text-gray-600">The parent information has been saved.</p>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-3 pb-4 border-b border-gray-200">
        ...
        <div>
          ...
          <p className="text-sm text-gray-600">Update the information for "{extraObject.parent.firstName} {extraObject.parent.lastName}"</p>
        </div>
      </div>
