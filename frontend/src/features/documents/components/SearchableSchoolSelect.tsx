...
const SearchableSchoolSelect: React.FC<SearchableSchoolSelectProps> = ({
  ...
  // Fetch schools when component mounts or regionId changes
  useEffect(() => {
      ...
      try {
        ...
        if (regionId) {
          response = showActiveOnly 
            ? await schoolApi.getActiveByRegionId(regionId)
            : await schoolApi.getByRegionId(regionId);
        } else {
          response = await schoolApi.getAll();
        }
        
        if (response.data.status === 'success' && response.data.data) {
          const schoolData = Array.isArray(response.data.data) 
            ? response.data.data 
            : [response.data.data];
          
          // Filter active schools if needed
          const filteredSchools = showActiveOnly && !regionId 
            ? schoolData.filter(school => school.active)
            : schoolData;
