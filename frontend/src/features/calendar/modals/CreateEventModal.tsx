...
import { createCalendarEvent, clearCalendarError } from '../calendarEventsSlice';
import { closeModal } from '../../common/modalSlice';
import { CreateCalendarEventRequest } from '../../../api/services/calendarEventApi';
...
const CreateEventModal: React.FC<CreateEventModalProps> = ({ extraObject }) => {
  ...
  useEffect(() => {
    if (extraObject?.selectedDate) {
      const date = extraObject.selectedDate;
      const time = extraObject.selectedTime || '09:00';
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await dispatch(createCalendarEvent(formData as CreateCalendarEventRequest)).unwrap();
      dispatch(closeModal({}));
    } catch (error) {
  ...
  const handleInputChange = (field: keyof CreateCalendarEventRequest, value: any) => {
        ...
        {error && (
          <div className="bg-red-50 border border-red-200 rounded-lg p-3">
            <div className="flex items-center gap-2 text-red-800">
              <AlertCircle size={16} />
              <span className="text-sm font-medium">Error creating event</span>
            </div>
